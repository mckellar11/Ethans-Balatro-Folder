...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float t = tentacle.y*2.221 + real_time*99.0 + time*199.0;
	...
	float r=sqrt(r2);
	float r8=r2*r2*r2*r2*16.0;
	float angle=atan(uv.y,uv.x)*3.0+ sin(real_time)+field*2.5*r ;
	float ray=cos(angle);//-mod(real_time, .2)
	float ray4=ray * ray ;
	if (1.0-ray4 > 0.001/(r2*r2)-clamp(0.25*(r-0.62),0.0,0.3))
	ray4 = ray4/8.;
    float res = (1.0 - (.4-clamp(0.03/r2,0.0,0.4))* 
		ray4 *2.0 
		);
